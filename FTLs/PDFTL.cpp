 ...
 *      Author: weixie
 *      Create a copy of the DFTL and add Parallel Map Loading and Write Back
 */
...
using namespace ssd;
int PDFTL::ENTRIES_PER_TRANSLATION_PAGE = 1024;
bool PDFTL::SEPERATE_MAPPING_PAGES = true;

PDFTL::PDFTL(Ssd *ssd, Block_manager_parent* bm) :
...
void PDFTL::read(Event *event) {
	...
	if (ongoing_mapping_operations.count(NUMBER_OF_ADDRESSABLE_PAGES() - translation_page_id) == 1) { // the translation page exist
		application_ios_waiting_for_translation[translation_page_id].push_back(event); // add current event to the transpage to Event map
	}
	else {
		//printf("creating mapping read %d for app write %d\n", translation_page_id, event->get_logical_address());
		create_mapping_read(translation_page_id, event->get_current_time(), event);  //if the mapping entry does not exist, create a new entry
	}
}

void PDFTL::register_read_completion(Event const& event, enum status result) {
...
void PDFTL::notify_garbage_collector(int translation_page_id, double time) {
	if (gc == NULL) {
		return;
	}
...
void PDFTL::register_write_completion(Event const& event, enum status result) {
...
void PDFTL::trim(Event *event)
{
	// For now we don't handle trims for PDFTL
	assert(false);
}

void PDFTL::register_trim_completion(Event & event) {
	page_mapping->register_trim_completion(event);
}

long PDFTL::get_logical_address(uint physical_address) const {
	return page_mapping->get_logical_address(physical_address);
}

Address PDFTL::get_physical_address(uint logical_address) const {
	return page_mapping->get_physical_address(logical_address);
}

void PDFTL::set_replace_address(Event& event) const {
	if (event.is_mapping_op()) {
...
void PDFTL::set_read_address(Event& event) const {
	if (event.is_mapping_op()) {
		Address ra = page_mapping->get_physical_address(event.get_logical_address());
...
void PDFTL::mark_clean(long translation_page_id, Event const& event) {
...
void PDFTL::try_clear_space_in_mapping_cache(double time) {
...
void PDFTL::create_mapping_read(long translation_page_id, double time, Event* dependant) {
